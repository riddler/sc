defmodule SC.Parser.SCXML.StateStackTest do
  use ExUnit.Case

  alias SC.Parser.SCXML.StateStack
  alias SC.{State, Document}
  ...
  describe "handle_state_end/1" do
    ...
    test "handles deeply nested state" do
      ...
      assert added_child.depth == 2  # Three levels deep: top -> mid -> child
    end
  end
  ...
  describe "handle_transition_end/1" do
    test "adds transition to state" do
      transition = %SC.Transition{event: "test_event", target: "target_state"}
    ...
    test "adds transition to parallel state" do
      transition = %SC.Transition{event: "parallel_event", target: "target_state"}
    ...
    test "adds transition to final state" do
      transition = %SC.Transition{event: "final_event", target: "target_state"}
  ...
  describe "handle_data_end/1" do
    test "adds data element to document datamodel" do
      data_element = %SC.DataElement{id: "test_data", expr: "value"}
      existing_document = %Document{datamodel_elements: [%SC.DataElement{id: "existing", expr: "old"}]}
      
      parsing_state = %{
        stack: [
          {"data", data_element},
          {"datamodel", nil},  # Placeholder for datamodel
    ...
    test "handles data element with non-datamodel parent" do
      data_element = %SC.DataElement{id: "orphan_data", expr: "value"}

defmodule SC.StateChartTest do
  use ExUnit.Case, async: true

  alias SC.{StateChart, Document, Configuration, Event}
  ...
  describe "new/2" do
    test "creates state chart with specific configuration", %{document: document, configuration: configuration} do
  ...
  describe "enqueue_event/2" do
    test "enqueues external events to external queue", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
    ...
    test "enqueues internal events to internal queue", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
    ...
    test "maintains order when enqueuing multiple events", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
      event1 = Event.new("event1")
      event2 = Event.new("event2")
      event3 = Event.internal("event3")
      
      updated_chart = state_chart
  ...
  describe "dequeue_event/1" do
    test "returns nil and unchanged chart when no events", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
    ...
    test "dequeues from external queue when internal queue is empty", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
      external_event = Event.new("external")
    ...
    test "maintains FIFO order for external events", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
      event1 = Event.new("event1")
      event2 = Event.new("event2")
  ...
  describe "has_events?/1" do
    test "returns false when no events in either queue", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
      refute StateChart.has_events?(state_chart)
    end

    test "returns true when internal queue has events", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
      event = Event.internal("internal_event")
    ...
    test "returns true when external queue has events", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
    ...
    test "returns true when both queues have events", %{document: document, configuration: configuration} do
      state_chart = StateChart.new(document, configuration)
  ...
  describe "active_states/1" do
    test "returns active states including ancestors", %{document: document, configuration: configuration} do

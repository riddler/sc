defmodule Mix.Tasks.Test.RegressionTest do
  ...
  describe "expand_test_patterns/1" do
    ...
    test "handles mixed pattern types" do
      patterns = [
        "#{@fixtures_dir}/sc_test.exs",           # direct file
        "#{@fixtures_dir}/sc/**/*_test.exs",     # wildcard
        "nonexistent/test.exs"                   # non-existent
      ]
      
      capture_io(:stderr, fn ->
        result = Regression.expand_test_patterns(patterns)
        
        assert length(result) == 3  # 1 direct + 2 wildcard matches
